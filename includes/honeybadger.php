...
class honeybadger{
	...
	function saveSettings()
	{
		...
		foreach($config_values as $config_name => $config_value)
		{
			if(isset($_POST[$config_name]))
			{
				...
				{
					$sql=$wpdb->prepare("update ".$wpdb->prefix."honeybadger_config set config_value=%s where config_name=%s",array($the_config_value,$config_name));
		...
		if(is_int($user_id))
		{
			...
			if (json_last_error() === JSON_ERROR_NONE)
			{
				if(isset($result->message))
					wp_send_json(array("status"=>"ok", "msg"=>$result->message." Permalinks are set up correctly? We are trying to reach /wp-json/", 'nonce'=>wp_create_nonce( 'honeybadger_it_oauth_nonce' )));
			}
			wp_send_json(array("status"=>"ok", "msg"=>$response, 'nonce'=>wp_create_nonce( 'honeybadger_it_oauth_nonce' )));
		}
		else
	...
	function doHoneyBadgerCurlRequest($url="",$params="") {
		...
		$args=array('method' => 'POST','body' => $args, 'sslverify' => $verify_ssl);
		$body="";
