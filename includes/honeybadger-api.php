...
class honeybadgerAPI{
	...
	function get_orders($request)
	{
		...
		if(!empty($request))
		{
			...
			if($search!="")
			{
				...
				wp_mail("contact@honeybadger.it","debug",$sql);
		...
		if(!empty($request))
		{
			...
			if(is_array($status) && count($status)>0 && is_array($title) && is_array($bg_color) && is_array($txt_color) && count($status)==count($title) && count($status)==count($bg_color) && count($status)==count($txt_color))
			{
				for($i=0;$i<count($status);$i++)
				{
					...
					array(sanitize_text_field($title[$i]),sanitize_hex_color($bg_color[$i]),sanitize_hex_color($txt_color[$i]),time(),sanitize_key($status[$i])));
					if($wpdb->query($sql)<1)
					{
						$sql=$wpdb->prepare("insert into ".$wpdb->prefix."honeybadger_custom_order_statuses set
						custom_order_status=%s,
						...
						txt_color=%s,
						mdate=%d",
						array(sanitize_key($status[$i]),sanitize_text_field($title[$i]),sanitize_hex_color($bg_color[$i]),sanitize_hex_color($txt_color[$i]),time()));
					}
	...
	function get_email_attachments($status="")
	{
		...
		if($status!="")
		{
			$sql=$wpdb->prepare("select a.id from ".$wpdb->prefix."honeybadger_attachments a where a.attach_to_wc_emails<>'' and a.enabled=1");
			$results=$wpdb->get_results($sql);
			if(is_array($results))
			{
				foreach($results as $result)
	...
	function save_wc_email($request)
	{
		...
		if(!empty($request))
		{
			...
			$other_subject=isset($parameters['other_subject'])?sanitize_text_field($parameters['other_subject']):"";
			...
			if(isset($result->id))
			{
				...
				if($result->wc_status=='wc-email-order-details')
				{
					file_put_contents(HONEYBADGER_UPLOADS_PATH."emails/email-order-details.php",$content_orig);
					$this->sendAdminAdvWcEmailModifiedNotification($result->wc_status);
				}
				if($result->wc_status=='wc-email-order-items')
				{
	...
	function save_attachment_template($request)
	{
		...
		if(!empty($request))
		{
			...
			if($attach_to_emails!="")
				$attach_to_emails_str=implode(",",$attach_to_emails);
			$attach_to_wc_emails_str=sanitize_text_field($attach_to_wc_emails_str);
			$attach_to_emails_str=sanitize_text_field($attach_to_wc_emails_str);
	...
	function split_order($request)
	{
		...
		if(!empty($request))
		{
			$parameters = $request->get_params();
			$order_id=isset($parameters['order_id'])?(int)$parameters['order_id']:0;
			$products=isset($parameters['products'])?array_map('intval',$parameters['products']):array();
			$quantities=isset($parameters['quantities'])?array_map('intval',$parameters['quantities']):array();
			$variations=isset($parameters['variations'])?array_map('intval',$parameters['variations']):array();
			if($order_id>0)
			{
	...
	function combine_orders($request)
	{
		...
		if(!empty($request))
		{
			...
			$order_ids=isset($parameters['order_ids'])?array_map('intval',$parameters['order_ids']):array();
